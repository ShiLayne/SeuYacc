...
fun_declaration	:	type	NAME	{ /* this is $3 */ $<sym>$ = symtab_insert(scope,$2,0); scope = symtab_open(scope); /* open new scope */ scope->function = $<sym>$; /* attach to this function */ }	LPAR	parameters	RPAR	{  /* this is $7 */  $<sym>3->type = types_fun($1,$5); }	block	{ scope = scope->parent; }
		;
...
more_parameters	:	parameter	COMMA	more_parameters	{ $$ = types_list_insert($3,$1); }	|	parameter				{ $$ = types_list_insert(0,$1); }
		;

parameter	:	type	NAME	{ symtab_insert(scope,$2,$1); /* insert in symbol table */ $$ = $1; /* remember type info */ }
		;

block		:	LBRACE		{ scope = symtab_open(scope); }
	var_declarations	statements	RBRACE					{ scope = scope->parent; /* close scope */}
		;

var_declarations:	var_declaration	var_declarations						|	/* empty */						
		;
...
type		:	INT			{ $$ = types_simple(int_t); }
		...
		|	STRUCT	LBRACE	fields	RBRACE	{ $$ = types_record($3);}						;
